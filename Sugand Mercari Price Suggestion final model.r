...
# Exploratory analysis of character variables
...
#colnames(target.train)


mercari.train <- train[,-c(6,9)]

#We'll remove the categories from here too as it's not helpful for our analysis having it here.
#mercari.train <- mercari.train[,-c(8,9,10)]
...
colnames(mr.fn) <- c('name','cat','brand','desc')
...
#In cell 84,From the word cloud and barplot of "name", 
...
class(mercari.df2)
#mercari.df2 <- as.data.frame(mercari.df2)
...
#mr.fn <- fread('mr.fn.csv')

colnames(mr.fn)

mr.fn2 <- mr.fn[,-c(1,2,3,4)]
...
testtarget <- fread('mercarisample_submission.csv')
#setwd('C:/Users/sugand1994/Documents')

#colnames(testtarget)
testtarget$log_price <- log(testtarget$price, base=exp(1))

#labels <- target.train$log_price
ts_label <- testtarget$log_price
...
new_tr <- model.matrix(~.,data = train_x,with=F)
#write.csv(train_x, file = 'train_x.csv')

new_ts <- model.matrix(~.,data = test_x,with=F)
...
params <- list(booster = "gbtree", objective = "reg:linear", eta=0.3, gamma=0, max_depth=6, 
               min_child_weight=1, subsample=1, colsample_bytree=1)

xgbcv <- xgb.cv( params = params, data = dtrain, nrounds = 700, nfold = 5, 
                showsd = T, stratified = T, print_every_n = 10, early_stopping_rounds = 10, maximize = F)


#model training
xgb1 <- xgb.train (params = params, data = dtrain, nrounds = 700, watchlist = list(val=dtest,train=dtrain),
                   print_every_n = 10, early_stopping_rounds = 10, maximize = F , eval_metric = "rmse")

xgbpred <- predict (xgb1,dtrain)

RMSLE(target.train$log_price, xgbpred)

xgbpredtr <- predict (xgb1,dtest)

RMSLE(testtarget$log_price, xgbpredtr)

mat <- xgb.importance (feature_names = colnames(new_tr),model = xgb1)
xgb.plot.importance (importance_matrix = mat[1:20]) 

mercari.df4 <- select(mercari.df3,c('category_name','brand_name','name','shipping','lularoe','item_condition_id','len_des',
                                   'new','bundle','bag','apple','leather','id','tags','michaekors','size','nike','reserved',
                                    'cond','pink'))

#train_x <- mercari.df4[1:train_len,]
#test_x <- mercari.df4[(train_len+1):mercari_len,]
#write.csv(train_x, file = 'train_x.csv')
#write.csv(test_x, file = 'test_x.csv')
write.csv(target.train, file = 'target.train.csv')
...
xgbcv2 <- xgb.cv( params = params, data = dtrain, nrounds = 500, nfold = 5, 
                showsd = T, stratified = T, print_every_n = 10, early_stopping_rounds = 10, maximize = F)


xgbcv2$best_iteration

params <- list(booster = "gbtree", objective = "reg:linear", gamma=0, eta=0.3, max_depth=6, 
              min_child_weight=1, subsample=1, colsample_bytree=1)

xgb2 <- xgb.train (params = params, data = dtrain, nrounds = 700, watchlist = list(val=dtest,train=dtrain),
                   print_every_n = 10, early_stopping_rounds = 10, maximize = F , eval_metric = "rmse")

xgb2$best_score

xgbpred2 <- predict (xgb2,dtrain)

RMSLE(target.train$log_price,xgbpred2)

xgbpredtr2 <- predict (xgb2,dtest)

RMSLE(testtarget$log_price,xgbpredtr2)

head(exp(xgbpredtr2))

xgbpredtr2 <- as.data.frame(xgbpredtr2)

xgbpredtr2 <- exp(xgbpredtr2)

mercari.final <- cbind(test$id,xgbpredtr2)
...
write.csv(mercari.final, "mercari.final.model.csv", row.names = F)

params <- list(booster = "gbtree", objective = "reg:linear", gamma=2, eta=0.5, max_depth=7, 
              min_child_weight=1, subsample=1, colsample_bytree=1)

xgb3 <- xgb.train (params = params, data = dtrain, nrounds = 1200, watchlist = list(val=dtest,train=dtrain),
                   print_every_n = 10, early_stopping_rounds = 10, maximize = F , eval_metric = "rmse")

xgbpred <- predict (xgb3,dtrain)

RMSLE(target.train$log_price,xgbpred)

xgbpredtr <- predict (xgb3,dtest)

RMSLE(testtarget$log_price,xgbpredtr)

params <- list(booster = "gbtree", objective = "reg:linear", gamma=1, eta=0.4, max_depth=6, 
              min_child_weight=1, subsample=1, colsample_bytree=1)

xgb3 <- xgb.train (params = params, data = dtrain, nrounds = 1200, watchlist = list(val=dtest,train=dtrain),
                   print_every_n = 10, early_stopping_rounds = 20, maximize = F , eval_metric = "rmse")

xgbpred <- predict (xgb3,dtrain)

RMSLE(target.train$log_price,xgbpred)

xgbpred <- predict (xgb3,dtest)

RMSLE(testtarget$log_price,xgbpred)
